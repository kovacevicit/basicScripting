...
SCRIPT_DIR="/home/akovac/scripts/"
...
if [ "$PRIORITY" = "HIGH" ]; then 
	 MSG=""
	  if [ "$Loss" = "100" ]; then
		      echo " "$TIME. Adresa $SITE je nedostupna" "Adresa $SITE je nedostupna"" | mail -s "Error ping" aleksandar.kovacevic@fisglobal.com
		       else
			          if [ "$Loss" -gt "8" ]; then
							        ...
							        MSG=$MSG"PING JE LOS. Gubitak paketa $LOSS. Ping - $Ping . "
								      ...
								      if [ "$MSG" != "" ];then
										      ...
										      SUBJ="$TIME - Check Internet Statistics for $SITE"
										           echo "$MSG" | mail -s "$SUBJ" aleksandar.kovacevic@fisglobal.com
											      fi
											       fi
fi
